...
"""
        if self.workload_type == "memory_intensive":
            if energy_savings > 5 and abs(ipc_change) < 3:
                report += "✅ The DVFS controller is effective: it reduces energy significantly with negligible performance loss — ideal for memory-bound apps."
            elif energy_savings > 0 and ipc_change > -5:
                report += "🟡 The DVFS controller saves energy with acceptable IPC trade-off."
            else:
                report += "❌ The DVFS controller is too aggressive — it boosts frequency unnecessarily, wasting power without improving performance."
        else:
